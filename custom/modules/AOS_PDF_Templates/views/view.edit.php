...
class AOS_PDF_TemplatesViewEdit extends ViewEdit {
    ...
    function setFields(){
        ...
        //Add all the modules to this list
        $complete_module_list = array_intersect($GLOBALS['moduleList'],array_keys($GLOBALS['beanList']));
        foreach($complete_module_list as $module_name) {
            $bean = BeanFactory::getBean($module_name);
            if(!isset($app_list_strings['pdf_template_type_dom'][$module_name])){
                if(isset($app_list_strings['moduleList'][$bean->getObjectName()])){
                    $app_list_strings['pdf_template_type_dom'][$bean->module_name] = $app_list_strings['moduleList'][$bean->getObjectName()];
                }
            }
        }
        asort($app_list_strings['pdf_template_type_dom']);
        //Add all the modules to this list
